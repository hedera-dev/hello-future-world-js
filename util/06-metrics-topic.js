...
async function initMetricsTopic() {
    ...
    await logger.logSectionWithWaitPrompt('Creating new topic with memo:', metricsHcsTopicMemo);
    console.log('Creating new topic with memo:', metricsHcsTopicMemo);
    const updatedLoggerConfig = await metricsTopicCreate(logger, metricsHcsTopicMemo);
    ...
    const fileContents = JSON.stringify({ config: loggerSampleConfig }, undefined, 2);
